"""Orchestrator agent: reasoning loop using Google Antigravity SDK."""
...
from google.antigravity import Agent, LocalAgentConfig, GeminiConfig, ModelConfig, ModelEntry, types
...
async def _run_agent_async(
    ...
    def query_sql(question: str, table_name: str) -> dict:
    ...
    def query_nosql(question: str, collection_name: str) -> dict:
    ...
    def search_pdfs(question: str, pdf_name: str) -> dict:
    ...
    async def pre_turn_hook(data: str) -> types.HookResult:
        ...
        if verbose:
            print(f"  [Orchestrator] Turn {current_turn}...")
    ...
    gemini_config = GeminiConfig(
        ...
        models=ModelConfig(
            default=ModelEntry(name=GEMINI_MODEL)
        )
    )

    system_prompt = _build_system_prompt()
    config = LocalAgentConfig(
        tools=[query_sql, query_nosql, search_pdfs],
...
def run_agent(question: str, verbose: bool = False) -> OrchestratorResult:
    return run_in_new_loop(_run_agent_async(question, verbose=verbose))
...
def run_agent_stream(question: str) -> Generator[str, None, None]:
    ...
    def run_graph():
        try:
            final_res = run_in_new_loop(
                _run_agent_async(question, callback=callback, verbose=False)
            )
            sources_consulted = [
                {
                    "source": r["source"],
